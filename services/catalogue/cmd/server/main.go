...
import (
	...
	catdb "github.com/activialtd/gomarketi.com-backend/services/catalogue/db"
	"github.com/activialtd/gomarketi.com-backend/services/catalogue/internal/handler"
...
func run(log zerolog.Logger) error {
	...
	if err := catdb.Migrate(ctx, db); err != nil {
	...
	go func() {
		log.Info().Str("addr", srv.Addr).Msg("catalogue service listening")
...
func connectDB(dsn string, log zerolog.Logger) (*sqlx.DB, error) {
	...
	for i := 1; i <= 5; i++ {
		...
		cancel()
		if err == nil {
			log.Info().Msg("catalogue db connected")
			return db, nil
		}
